import Link from 'next/link'
import type { ReactNode } from 'react'
import { ArrowRight } from 'lucide-react'
import { PortalBadge } from '@/components/ui/portal-badge'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { Button } from '@/components/ui/button'

export interface SignInShellProps {
  variant: 'client' | 'lawyer' | 'admin'
  eyebrow: string
  headline: ReactNode
  subheadline: string
  formTitle: string
  formSubtitle: string
  footer?: ReactNode
  errorMessage?: string
  action: (formData: FormData) => void | Promise<void>
}

const paletteFor = (v: SignInShellProps['variant']) => {
  switch (v) {
    case 'admin':
      return { panelBg: '#101014', eyebrowColor: '#d98da4' }
    case 'lawyer':
      return { panelBg: '#1a2744', eyebrowColor: '#c9a96a' }
    default:
      return { panelBg: '#1a2744', eyebrowColor: '#d98da4' }
  }
}

export function SignInShell({
  variant,
  eyebrow,
  headline,
  subheadline,
  formTitle,
  formSubtitle,
  footer,
  errorMessage,
  action,
}: SignInShellProps) {
  const p = paletteFor(variant)

  return (
    <main className="flex-1 grid lg:grid-cols-[1.1fr_1fr] min-h-screen">
      {/* Night band — the single dark marketing surface */}
      <section
        className="relative hidden lg:flex flex-col justify-between overflow-hidden p-16 text-cream"
        style={{ background: p.panelBg }}
      >
        <div className="relative z-10 flex items-center gap-3">
          <div
            className="h-px w-8"
            style={{ background: p.eyebrowColor }}
          />
          <span
            className="font-sans text-eyebrow uppercase tracking-[0.2em]"
            style={{ color: p.eyebrowColor }}
          >
            {eyebrow}
          </span>
        </div>

        <div className="relative z-10 max-w-lg">
          <h2
            className="font-serif font-semibold text-white leading-[1.12] mb-6"
            style={{ fontSize: 'clamp(2.25rem, 3.5vw, 3.25rem)' }}
          >
            {headline}
          </h2>
          <p className="font-sans text-body leading-relaxed text-white/80">
            {subheadline}
          </p>
        </div>

        <div className="relative z-10 flex items-end justify-between text-white/60">
          <span className="font-serif italic text-caption">
            Nairobi · Kenya
          </span>
          <span className="font-sans text-eyebrow uppercase tracking-widest">
            CBK · ODPC
          </span>
        </div>
      </section>

      {/* Form panel — auth card on cream canvas */}
      <section className="flex items-center justify-center p-6 sm:p-12 bg-surface">
        <div className="w-full max-w-md">
          <div className="bg-white border border-hairline rounded-xl shadow-soft p-8 sm:p-10">
            <div className="flex items-center justify-between gap-3 mb-8">
              <span className="font-sans text-eyebrow uppercase tracking-[0.15em] text-accent">
                {formTitle}
              </span>
              <PortalBadge variant={variant} tone="light" />
            </div>

            <h1 className="text-h2 font-serif text-primary mb-2">
              Sign in to RegWatch
            </h1>
            <p className="font-sans text-body-sm text-ink-muted mb-8">
              {formSubtitle}
            </p>

            {errorMessage && (
              <div
                className="mb-6 px-4 py-3 rounded-md bg-error/[0.06] border border-error/20 font-sans text-caption text-error"
                role="alert"
              >
                {errorMessage}
              </div>
            )}

            <form className="flex flex-col gap-5">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="email">Work email</Label>
                <Input
                  id="email"
                  name="email"
                  type="email"
                  required
                  autoComplete="email"
                  placeholder="name@company.com"
                />
              </div>

              <div className="flex flex-col gap-1.5">
                <Label htmlFor="password">Password</Label>
                <Input
                  id="password"
                  name="password"
                  type="password"
                  required
                  autoComplete="current-password"
                  placeholder="••••••••"
                />
              </div>

              <Button formAction={action} size="lg" className="mt-2 w-full">
                Sign in
                <ArrowRight className="w-4 h-4" />
              </Button>
            </form>
          </div>

          {footer && (
            <div className="mt-8 px-2 font-sans text-caption text-ink-muted space-y-2">
              {footer}
            </div>
          )}
        </div>
      </section>
    </main>
  )
}

export function ShellFooterLink({
  href,
  children,
}: {
  href: string
  children: ReactNode
}) {
  return (
    <Link
      href={href}
      className="text-accent font-medium underline underline-offset-4 transition-colors hover:text-accent-active"
    >
      {children}
    </Link>
  )
}
